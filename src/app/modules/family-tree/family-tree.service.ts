...
import {  Family } from 'app/modules/family-tree/family-tree.types';
...
// private Family relationships
 ...
 private _familyMembers: BehaviorSubject<FamiliesMembers | null> = new BehaviorSubject(null);
 private _familiesMembers: BehaviorSubject<FamiliesMembers[] | null> = new BehaviorSubject(null);
 private _unsubscribeAllMembers: Subject<any> = new Subject<any>();
    ...
    createRelationship(type: string): Observable<Family>
    {
        ...
        return this.families$.pipe(
            take(1),
            switchMap(bequests => this._httpClient.post<Family>('api/family-tree/family', {type}).pipe(
                map((newBequest) =>
                {
       ...
       getFamilyById(id: string): Observable<Family>
       {
           return this._families.pipe(
               ...
               switchMap((bequest) =>
               {
                   if ( !bequest )
                   {
                       return throwError('Could not found bequest with id of ' + id + '!');
                   }
...
get familyMembers$(): Observable<FamiliesMembers>
    ...
    createMember(type: string): Observable<Family>
    {
        ...
        return this.familiesMembers$.pipe(
            take(1),
            switchMap(Members => this._httpClient.post<FamiliesMembers>('api/family-tree/family-members', {type}).pipe(
                map((newMember) =>
                {
       ...
       getFamilyMembersById(id: string): Observable<FamiliesMembers>
       {
           return this._familiesMembers.pipe(
               ...
               map((Members) =>
               {
                   ...
                   // Update the Members
                   this._familiesMembers.next([Member]);
               ...
               switchMap((Member) =>
               {
                   if ( !Member )
                   {
                       return throwError('Could not found Members with id of ' + id + '!');
                   }
     ...
     * @param bequest
     */
updateFamilyMembers(id: string, familyMember: FamiliesMembers): Observable<FamiliesMembers>
{
    return this.familiesMembers$
        .pipe(
            take(1),
            switchMap(familyMembers => this._httpClient.patch<FamiliesMembers>('api/family-tree/family-members', {
                id,
                familyMember,
            }).pipe(
                map((updateFamilyMembers) =>
                {
                    // Find the index of the updated bequest
                    const index = familyMembers.findIndex(item => item.id === id);

                    // Update the bequest
                    familyMembers[index] = updateFamilyMembers;
                ...
                switchMap(updateFamilyMembers => this.familyMembers$.pipe(
                    take(1),
                    filter(item => item && item.id === id),
                    tap(() =>
                    {
                        // Update the bequest if it's selected
                        this._familyMembers.next(updateFamilyMembers);
